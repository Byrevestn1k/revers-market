...
type Conversation = { id: string; orderId?: string | null; offerId?: string | null; productId?: string | null; buyRequestId?: string | null; type: 'direct' | 'offer' | 'deal'; status: string; title?: string; price?: number | null; currency?: string | null; imageUrl?: string | null; otherUserId?: string; otherUsername: string; otherAvatarUrl?: string | null; userRole?: 'selling' | 'buying'; lastMessage: string | null; lastMessageSenderId?: string | null; lastMessageIsMine?: boolean; lastMessageAt: string | null; lastReadAt: string | null; unreadCount?: number }
...
function RequestsMarket({ mine, notify }: { mine: Product[]; notify: (message: string) => void }) {
    ...
    const openRequestChat = async (item: BuyRequest) => {
    ...
    return <section className="content"><div className="view-header"><div><span className="eyebrow">Попит</span><h1>Запити покупців</h1><p className="view-subtitle">Відгукніться своєю пропозицією на запити поруч.</p></div><button className="outline-button" onClick={load}>↻ Оновити</button></div>
        ...
        <div className="request-list">{myOffers.map((offer) => <article key={offer.id} className="request-card"><header><div><span className="eyebrow">Пропозиція</span><h3>{offer.existingProduct?.title ?? 'Без прив’язки до товару'}</h3></div><span className="status status-paused">{OFFER_STATUS[offer.status] ?? offer.status}</span></header>
            {offer.pendingNegotiation && <p className="form-hint">? ???? ????? ??? ???????. ??????????? ?? ? ????.</p>}{offer.waitingConfirmation && <p><a href="/?view=orders">???????? ????? ???? ??????????. ??????????? ????????????.</a></p>}<div className="card-meta"><span>{offer.quantity} {offer.unit}</span><span>{formatPrice(offer.price.amount, offer.price.currency)} / {offer.unit}</span><span>Прийнято: {offer.acceptedQuantity}</span></div>
...
function OrdersView({ notify }: { notify: (message: string) => void }) {
    ...
    const confirmDeal = async (order: Order) => { if (busy) return; setBusy(true); try { await request(`/api/orders/${order.id}/seller-confirm`, { method: 'POST', body: '{}' }); notify('Домовленість підтверджено'); load() } catch (error) { notify((error as Error).message) } finally { setBusy(false) } }
    return <section className="content"><div className="view-header"><div><span className="eyebrow">Угоди</span><h1>Мої домовленості</h1><p className="view-subtitle">Узгодьте передачу товару, підтвердьте результат і залиште відгук.</p></div><button className="outline-button" onClick={load}>↻ Оновити</button></div>
        <div className="message-scope-tabs">{[['active','Активні'],['completed','Завершені'],['cancelled','Скасовані'],['all','Усі']].map(([value,label]) => <button key={value} className={scope === value ? 'active' : ''} onClick={() => setScope(value)}>{label}</button>)}</div>
        <div className="order-list">{orders.filter(order => scope === 'all' || (scope === 'active' ? !['completed','failed','cancelled','rejected','expired'].includes(order.status) : scope === 'completed' ? order.status === 'completed' : ['failed','cancelled','rejected','expired'].includes(order.status))).map((order) => <article key={order.id} className="order-card"><header><div><span className="eyebrow">{order.buyer.username} ↔ {order.seller.username}</span><h3>{order.conditionsSnapshot?.productTitle || 'Замовлення'}</h3></div><span className="status status-active">{ORDER_STATUS[order.status] ?? 'Домовленість'}</span></header>
            <p>{dealStatusText(order, viewerId)}</p><div className="deal-timeline"><span className="done">✓ Пропозицію обрано</span><span className={order.sellerConfirmedAt || ['in_progress','buyer_marked_completed','seller_marked_completed','completed'].includes(order.status) ? 'done' : ''}>Продавець підтвердив</span><span className={order.status === 'completed' ? 'done' : ''}>Результат угоди</span><span className={order.myReviewCreated ? 'done' : ''}>Відгук</span></div><div className="card-meta"><span>{order.quantity ?? '—'} {order.unit}</span><span>{formatPrice(order.price.unit ?? 0, order.price.currency)} / {order.unit}</span><span>{formatPrice(order.subtotal, order.price.currency)}</span></div>
            {order.actualQuantity != null && <p>Фактично отримано: {order.actualQuantity} {unitLabel(order.unit)} · {formatPrice(order.actualTotal ?? order.subtotal, order.price.currency)}</p>}
            {order.failureReason && <p>Причина: {failureReasons[order.failureReason.split(':')[0]] ?? 'Інше'}{order.failureReason.includes(':') ? ': ' + order.failureReason.split(':').slice(1).join(':') : ''}</p>}
            {order.conditionsSnapshot?.delivery?.delivery && <p>Отримання: {order.conditionsSnapshot.delivery.delivery}</p>}
            {order.cancelReason && <p className="form-hint">Причина скасування: {order.cancelReason}</p>}
            {order.dispute && <div>{[['????????', order.buyerResult], ['?????????', order.sellerResult]].map(([label, result]) => result && <p key={label}>{label}: {result.outcome === 'completed' ? `${result.quantity} ${unitLabel(order.unit)}, ${formatPrice(result.total, order.price.currency)}` : '????? ?? ?????????'}{result.resolutionOutcome ? ' ? ????????????? ???????? ?????????' : ''}</p>)}</div>}{order.dispute && <p className={order.dispute.status === 'open' ? 'form-error' : 'form-hint'}>Узгодження результату: {order.dispute.reason}{order.dispute.resolution ? ` — Рішення: ${order.dispute.resolution}` : ''}</p>}
            <div className="request-actions">
                {order.status === 'selected' && order.seller.id === viewerId && <button className="primary-button compact" disabled={busy} onClick={() => confirmDeal(order)}>Підтвердити домовленість</button>}
                {order.status === 'accepted' && order.seller.id === viewerId && <button className="primary-button compact" onClick={() => setStatus(order, 'in_progress')}>▶ Розпочати</button>}
