...
function Auth({ onLogin }: { onLogin: (user: User) => void }) {
    ...
    if (resetMode) return <main className="auth-page"><section className="auth-card"><h2>Відновлення паролю</h2><p>Введіть логін або email, щоб отримати 4-значний код.</p><input name="resetIdentifier" placeholder="Логін або email" />{resetSent && <><input value={resetCode} onChange={(event) => setResetCode(event.target.value.replace(/\D/g, '').slice(0, 4))} placeholder="4-значний код" inputMode="numeric" /><input name="newPassword" type="password" placeholder="Новий пароль" /></>}<button className="primary-button" onClick={async () => { const identifier = (document.querySelector('input[name="resetIdentifier"]') as HTMLInputElement)?.value; try { if (!resetSent) { await request('/api/auth/password-reset/request', { method: 'POST', body: JSON.stringify({ identifier }) }); setResetSent(true); } else { const password = (document.querySelector('input[name="newPassword"]') as HTMLInputElement)?.value; await request('/api/auth/password-reset/confirm', { method: 'POST', body: JSON.stringify({ identifier, code: resetCode, password }) }); setResetMode(false); setResetSent(false); notify('Пароль змінено') } } catch (error) { setError((error as Error).message) } }}>{resetSent ? 'Змінити пароль' : 'Надіслати код'}</button><button className="text-button" onClick={() => { setResetMode(false); setResetSent(false) }}>Повернутися до входу</button>{error && <p className="form-error">{error}</p>}</section></main>
...
function ProfileView({ logout, notify }: { logout: () => void; notify: (message: string) => void }) {
    ...
    return <section className="content profile-view"><span className="eyebrow">Налаштування</span><h1>Налаштування профілю</h1>
        ...
        <form className="product-form" onSubmit={save}>
            ...
            <label>Телефон<div className="phone-display"><input value={profile.phone} readOnly /><button type="button" className="avatar-edit inline-edit" onClick={() => setEditingPhone(!editingPhone)}>✎</button></div>{editingPhone && <div className="phone-editor"><PhoneInput countryCode={phoneCountry} phone={profile.phone} onCountryCode={setPhoneCountry} onPhone={(value) => setProfile({ ...profile, phone: value })} /><button type="button" className="outline-button compact" onClick={async () => { try { await request('/api/profile/me/phone-verification', { method: 'POST', body: '{}' }); setPhoneCodeSent(true); notify('Код підтвердження виведено в термінал бекенду') } catch (error) { setMessage((error as Error).message) } }}>Надіслати SMS</button></div>}{phoneCodeSent && <div className="verification-popup-inline"><input value={phoneCode} onChange={(event) => setPhoneCode(event.target.value.replace(/\D/g, '').slice(0, 4))} inputMode="numeric" placeholder="4-значний код" /><button type="button" className="outline-button compact" disabled={phoneCode.length !== 4} onClick={async () => { try { await request('/api/profile/me/phone-verification/confirm', { method: 'POST', body: JSON.stringify({ code: phoneCode }) }); setPhoneCodeSent(false); setPhoneCode(''); setEditingPhone(false); notify('Телефон підтверджено') } catch (error) { setMessage((error as Error).message) } }}>Підтвердити</button></div>}</label>
            <label className={`email-field ${profile.emailVerified ? 'email-verified' : 'email-unverified'}`}>Електронна пошта <span className="email-status">({profile.emailVerified ? 'підтверджена' : 'не підтверджена'})</span><div className="phone-display"><input name="email" type="email" value={editingEmail ? emailDraft : (profile.email ?? '')} maxLength={254} readOnly={!editingEmail} onChange={(event) => setEmailDraft(event.target.value)} /><button type="button" className="avatar-edit inline-edit" onClick={() => { setEditingEmail(!editingEmail); setEmailDraft(profile.email ?? ''); setEmailCodeSent(false) }}>✎</button></div>{editingEmail && emailDraft.trim().toLowerCase() !== (profile.email ?? '').toLowerCase() && <button type="button" className="outline-button compact" onClick={async () => { try { await request('/api/profile/me', { method: 'PATCH', body: JSON.stringify({ email: emailDraft }) }); await request('/api/profile/me/email-verification', { method: 'POST', body: JSON.stringify({ email: emailDraft }) }); setEmailCodeSent(true); notify('Код підтвердження виведено в термінал бекенду') } catch (error) { setMessage((error as Error).message) } }}>{emailCodeSent ? 'Надіслати повторно' : 'Підтвердити email'}</button>}{emailCodeSent && <div className="verification-popup-inline"><input value={emailCode} onChange={(event) => setEmailCode(event.target.value.replace(/\D/g, '').slice(0, 4))} inputMode="numeric" placeholder="4-значний код" /><button type="button" className="outline-button compact" disabled={emailCode.length !== 4} onClick={async () => { try { await request('/api/profile/me/email-verification/confirm', { method: 'POST', body: JSON.stringify({ code: emailCode }) }); setEmailCodeSent(false); setEmailCode(''); setEditingEmail(false); const result = await request('/api/profile/me'); setProfile(result.profile); setEmailDraft(result.profile.email ?? ''); notify('Email підтверджено') } catch (error) { setMessage((error as Error).message) } }}>Підтвердити код</button></div>}</label>
            <div className="password-change"><strong>Пароль</strong><button type="button" className="avatar-edit inline-edit" onClick={() => { setEditingPassword(!editingPassword); setPasswordMessage('') }}>✎</button>{editingPassword && <div className="password-editor"><input name="currentPassword" type="password" placeholder="Поточний пароль" /><input name="newPassword" type="password" placeholder="Новий пароль" minLength={8} /><input name="passwordConfirmation" type="password" placeholder="Підтвердження нового паролю" minLength={8} /><button type="button" className="outline-button compact" onClick={async (event) => { const box = event.currentTarget.parentElement; const currentPassword = (box?.querySelector('[name=currentPassword]') as HTMLInputElement)?.value; const newPassword = (box?.querySelector('[name=newPassword]') as HTMLInputElement)?.value; const confirmation = (box?.querySelector('[name=passwordConfirmation]') as HTMLInputElement)?.value; try { await request('/api/profile/me/password', { method: 'POST', body: JSON.stringify({ currentPassword, newPassword, confirmation }) }); setEditingPassword(false); setPasswordMessage('Пароль змінено') } catch (error) { setPasswordMessage((error as Error).message) } }}>Змінити пароль</button>{passwordMessage && <span className="field-error">{passwordMessage}</span>}</div>}</div>
            <div className="field-row">
