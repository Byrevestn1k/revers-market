...
import type { HomeCity } from './PublicHome'
...
export type Product = { id: string; title: string; description: string; photos: { url: string; alt: string }[]; quantity: number; availableQuantity?: number; unit: string; price: { amount: number; currency: string }; deliveryMode: string; geoZone: string; status: string; owner: { id: string }; category: { id: string; name: string } }
...
type BuyRequest = { id: string; title: string; description: string; geoArea: string; category: { id: string; name: string }; quantity: number; fulfilledQuantity: number; unit: string; status: string; coordinates: MapPoint | null; buyer?: { id: string; username: string }; price: { min: number | null; max: number | null; currency: string }; delivery: { required: boolean; preferred: string | null; address?: string | null }; deadline: string | null }
...
function MapView({ categories, openProduct, notify, city, user, onManageRequest, onBrowseRequests }: { categories: Category[]; openProduct: (product: Product) => void; notify: (message: string) => void; city?: HomeCity; user?: User | null; onManageRequest?: () => void; onBrowseRequests?: () => void }) {
    ...
    useEffect(() => {
        if (city) { setCenter(city); setZoom(12); setGeoZone('') }
    }, [city])
...
function BuyRequestForm({ categories, buyRequest, copy = false, locked = false, onSaved, onCancel }: { categories: Category[]; buyRequest?: BuyRequest; copy?: boolean; locked?: boolean; onSaved: (buyRequest?: BuyRequest) => void; onCancel: () => void }) {
    const isEditing = Boolean(buyRequest && !copy)
    const [latitude, setLatitude] = useState(buyRequest?.coordinates?.latitude?.toString() ?? '50.4506')
    const [longitude, setLongitude] = useState(buyRequest?.coordinates?.longitude?.toString() ?? '30.5239')
    const [categoryId, setCategoryId] = useState(buyRequest?.category.id ?? '')
    const [busy, setBusy] = useState(false)
    const [error, setError] = useState('')
    const [fieldErrors, setFieldErrors] = useState<Record<string, string>>({})
    ...
    const submit = async (event: FormEvent<HTMLFormElement>) => {
        event.preventDefault(); setBusy(true); setError(''); setFieldErrors({})
        const data = Object.fromEntries(new FormData(event.currentTarget).entries()) as Record<string, string>
        const minPrice = Number(data.minPrice); const maxPrice = Number(data.maxPrice)
        const payload = { categoryId: data.categoryId || categoryId, title: data.title.trim(), description: data.description ?? '', quantity: Number(data.quantity), unit: data.unit, currency: data.currency, minPrice, maxPrice, delivery: data.delivery, preferredDelivery: data.preferredDelivery || null, geoArea: data.geoArea.trim(), address: data.address.trim() || null, latitude: latitude.trim() ? Number(latitude) : undefined, longitude: longitude.trim() ? Number(longitude) : undefined, deadline: data.deadline ? new Date(`${data.deadline}T23:59:59.000Z`).toISOString() : null }
        if (isEditing && locked) { delete (payload as Partial<typeof payload>).categoryId; delete (payload as Partial<typeof payload>).quantity; delete (payload as Partial<typeof payload>).unit; delete (payload as Partial<typeof payload>).currency }
        const localErrors: Record<string, string> = {}
        if (payload.title.length < 2) localErrors.title = 'Введіть назву щонайменше з 2 символів'
        if (!isEditing || !locked) {
            if (!payload.categoryId) localErrors.categoryId = 'Оберіть категорію'
            if (!Number.isFinite(payload.quantity) || payload.quantity <= 0) localErrors.quantity = 'Кількість має бути більшою за нуль'
        }
        if (!Number.isFinite(minPrice) || minPrice < 0) localErrors.minPrice = 'Вкажіть невід’ємну ціну'
        if (!Number.isFinite(maxPrice) || maxPrice < 0) localErrors.maxPrice = 'Вкажіть невід’ємну ціну'
        if (minPrice > maxPrice) { localErrors.minPrice = 'Мінімальна ціна не може бути більшою за максимальну'; localErrors.maxPrice = localErrors.minPrice }
        if (!payload.geoArea) localErrors.geoArea = 'Вкажіть регіон пошуку'
        if (Object.keys(localErrors).length) { setFieldErrors(localErrors); setBusy(false); return }
        try { const result = await request(isEditing ? `/api/buy-requests/${buyRequest!.id}` : '/api/buy-requests', { method: isEditing ? 'PATCH' : 'POST', body: JSON.stringify(payload) }); setDirty(false); onSaved(result.buyRequest) }
    ...
    const fieldError = (field: string) => fieldErrors[field] ? <span className="field-error">{fieldErrors[field]}</span> : null
    const deadline = !copy && buyRequest?.deadline ? buyRequest.deadline.slice(0, 10) : ''
    const leave = () => { if (!dirty || window.confirm('Введені зміни не збережені. Вийти з форми?')) onCancel() }
    return <section className="form-view"><div className="view-header"><div><span className="eyebrow">Попит</span><h1>{isEditing ? 'Редагувати запит' : copy ? 'Створити копію запиту' : 'Створити запит на купівлю'}</h1><p className="view-subtitle">Продавці поблизу побачать лише приблизну точку.</p></div><button className="icon-button" aria-label="Закрити форму" type="button" onClick={leave}>×</button></div><form className="product-form request-form" onChange={() => setDirty(true)} onSubmit={submit}><label className={fieldErrors.title ? 'field-invalid' : ''}>Що потрібно *<input name="title" defaultValue={buyRequest?.title} placeholder="Наприклад, пшениця для господарства" required />{fieldError('title')}</label><label className={fieldErrors.categoryId ? 'field-invalid' : ''}>Категорія *<select name="categoryId" value={categoryId} disabled={isEditing && locked} onChange={(event) => setCategoryId(event.target.value)} required><option value="" disabled>Оберіть категорію</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select>{isEditing && locked && <small>Категорію заблоковано після появи пропозицій.</small>}{fieldError('categoryId')}</label><label>Опис<textarea name="description" defaultValue={buyRequest?.description} rows={4} placeholder="Коротко опишіть вимоги" /></label><div className="field-row"><label>Кількість *<input name="quantity" disabled={isEditing && locked} type="number" min=".001" step=".001" defaultValue={buyRequest?.quantity ?? '1'} required />{fieldError('quantity')}</label><label>Одиниця *<select name="unit" disabled={isEditing && locked} defaultValue={buyRequest?.unit ?? 'kg'}><option value="kg">кг</option><option value="ton">тонна</option><option value="litre">літр</option><option value="piece">шт.</option><option value="box">ящик</option></select></label></div><div className="field-row"><label>Мінімальна ціна<input name="minPrice" type="number" min="0" step=".01" defaultValue={buyRequest?.price.min ?? 0} required />{fieldError('minPrice')}</label><label>Максимальна ціна<input name="maxPrice" type="number" min="0" step=".01" defaultValue={buyRequest?.price.max ?? 0} required />{fieldError('maxPrice')}</label></div><label>Валюта *<select name="currency" disabled={isEditing && locked} defaultValue={buyRequest?.price.currency ?? 'UAH'}><option>UAH</option><option>EUR</option><option>PLN</option></select></label><div className="field-row"><label>Доставка<select name="delivery" defaultValue={buyRequest ? (buyRequest.delivery.required ? (buyRequest.delivery.preferred ? 'preferred' : 'yes') : 'no') : 'preferred'}><option value="no">Не потрібна</option><option value="yes">Потрібна</option><option value="preferred">Бажана</option></select></label><label>До дати<input name="deadline" type="date" min={new Date().toISOString().slice(0, 10)} defaultValue={deadline} /></label></div><label>Регіон пошуку *<input name="geoArea" defaultValue={buyRequest?.geoArea} placeholder="Київ, Київська область" required />{fieldError('geoArea')}</label><label>Точна адреса для доставки <small className="form-hint">не показується на мапі</small><input name="address" defaultValue={buyRequest?.delivery.address ?? ''} placeholder="Необов’язково" /></label><div className="request-location"><div><strong>Приблизна точка на мапі</strong><small>Використовується тільки для пошуку поруч</small></div><button type="button" className="outline-button" onClick={useLocation}>⌖ Моє місце</button><div className="field-row"><label>Широта<input value={latitude} onChange={(event) => setLatitude(event.target.value)} type="number" step=".000001" min="-90" max="90" required /></label><label>Довгота<input value={longitude} onChange={(event) => setLongitude(event.target.value)} type="number" step=".000001" min="-180" max="180" required /></label></div></div>{error && <p className="form-error" role="alert">{error}</p>}<button className="primary-button" disabled={busy}>{busy ? 'Збереження…' : isEditing ? 'Зберегти зміни' : 'Опублікувати новий запит'} <span>→</span></button></form></section>
}
...
function App() {
    ...
    const requestSaved = () => { setView('requests'); notify('Запит опубліковано') }
    ...
    if (!user || view === 'home') return <><PublicHome request={request} user={user} onAccount={() => user ? go('profile') : enter()} onCreate={() => user ? go('create') : enter(true)} openProduct={setSelected} renderMap={(city) => <MapView categories={categories} city={city} openProduct={setSelected} notify={notify} />} />{selected && <ProductDetail product={selected} close={() => setSelected(null)} owner={selected.owner.id === user?.id} edit={() => { setEditing(selected); setSelected(null); setView('create') }} />}{toast && <div className="toast">{toast}</div>}</>
...
function ConfirmRequestCancellation({ item, onConfirm, onClose }: { item: BuyRequest; onConfirm: () => void; onClose: () => void }) {
    return <div className="modal-backdrop" role="presentation"><section className="detail-modal" role="dialog" aria-modal="true" aria-labelledby="cancel-request-title"><button className="modal-close" aria-label="Закрити" onClick={onClose}>×</button><div className="detail-body"><span className="eyebrow">Скасування запиту</span><h2 id="cancel-request-title">Скасувати «{item.title}»?</h2><p className="detail-description">Після скасування продавці більше не зможуть надсилати нові пропозиції. Скасований запит не відкривається повторно, але його дані можна використати для створення нового.</p><div className="request-actions"><button className="outline-button" onClick={onClose}>Не скасовувати</button><button className="primary-button" onClick={onConfirm}>Так, скасувати</button></div></div></section></div>
}

function RequestDetailsDialog({ item, onClose }: { item: BuyRequest; onClose: () => void }) {
    const delivery = item.delivery.required ? (item.delivery.preferred ? `Бажана: ${item.delivery.preferred}` : 'Потрібна') : 'Не потрібна'
    return <div className="modal-backdrop" role="presentation"><section className="detail-modal" role="dialog" aria-modal="true" aria-labelledby="request-details-title"><button className="modal-close" aria-label="Закрити деталі" onClick={onClose}>×</button><div className="detail-body"><span className="eyebrow">{item.category.name} · {REQUEST_STATUS[item.status] ?? item.status}</span><h2 id="request-details-title">{item.title}</h2><p className="detail-description">{item.description || 'Опис не додано.'}</p><div className="detail-facts"><span>▧ <b>{item.fulfilledQuantity}/{item.quantity} {item.unit}</b><small>Виконано / потрібно</small></span><span>₴ <b>{item.price.min ?? '—'}–{item.price.max ?? '—'} {item.price.currency}</b><small>Бюджет</small></span><span>♧ <b>{delivery}</b><small>Доставка</small></span><span>⌖ <b>{item.geoArea}</b><small>Регіон пошуку</small></span><span>◷ <b>{item.deadline ? new Date(item.deadline).toLocaleDateString('uk-UA') : 'Не вказано'}</b><small>Дедлайн</small></span>{item.delivery.address && <span>⌂ <b>{item.delivery.address}</b><small>Точна адреса — лише для власника</small></span>}</div></div></section></div>
}

function AcceptOfferDialog({ offer, buyRequest, onConfirm, onClose }: { offer: Offer; buyRequest: BuyRequest; onConfirm: (quantity: number) => Promise<void>; onClose: () => void }) {
    const maximum = Math.min(offer.quantity - offer.acceptedQuantity, buyRequest.quantity - buyRequest.fulfilledQuantity)
    const [quantity, setQuantity] = useState(maximum)
    const [error, setError] = useState('')
    const [busy, setBusy] = useState(false)
    const total = quantity * offer.price.amount
    const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!Number.isFinite(quantity) || quantity <= 0 || quantity > maximum) { setError(`Вкажіть кількість від 0.001 до ${maximum} ${offer.unit}`); return }; setBusy(true); try { await onConfirm(quantity) } catch (caught) { setError((caught as Error).message); setBusy(false) } }
    return <div className="modal-backdrop" role="presentation"><section className="detail-modal" role="dialog" aria-modal="true" aria-labelledby="accept-offer-title"><button className="modal-close" aria-label="Закрити" onClick={onClose}>×</button><div className="detail-body"><span className="eyebrow">Прийняття пропозиції</span><h2 id="accept-offer-title">Перевірте підсумок</h2><p className="detail-description">Продавець: <b>{offer.seller.username}</b><br />Ціна: <b>{formatPrice(offer.price.amount, offer.price.currency)}</b> за {offer.unit}<br />Доставка: <b>{offer.delivery}</b></p><form className="offer-form" onSubmit={submit}><label>Кількість ({offer.unit})<input autoFocus type="number" min=".001" step=".001" max={maximum} value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} required /></label><p>Загальна сума: <b>{formatPrice(total, offer.price.currency)}</b></p>{error && <p className="form-error" role="alert">{error}</p>}<div className="request-actions"><button type="button" className="outline-button" onClick={onClose}>Назад</button><button className="primary-button" disabled={busy}>{busy ? 'Приймаємо…' : 'Підтвердити'}</button></div></form></div></section></div>
}

function MyRequests({ categories, notify, create }: { categories: Category[]; notify: (message: string) => void; create: () => void }) {
    const [requests, setRequests] = useState<BuyRequest[]>([])
    ...
    const [cancelling, setCancelling] = useState<BuyRequest | null>(null)
    const [accepting, setAccepting] = useState<{ offer: Offer; buyRequest: BuyRequest } | null>(null)
    const [details, setDetails] = useState<BuyRequest | null>(null)
    const [chat, setChat] = useState<{ conversationId: string; title: string } | null>(null)
    const load = async () => { setLoading(true); setLoadError(''); try { setRequests((await request('/api/buy-requests?mine=true')).buyRequests) } catch (error) { const message = (error as Error).message; setLoadError(message); notify(message) } finally { setLoading(false) } }
    useEffect(() => { load() }, [])
    ...
    const accept = async (offer: Offer, quantity: number) => { await request(`/api/offers/${offer.id}/accept`, { method: 'POST', body: JSON.stringify({ quantity }) }); setAccepting(null); notify('Пропозицію прийнято — створено замовлення й оновлено прогрес запиту'); setExpanded(''); load() }
    const cancel = async (item: BuyRequest) => { await request(`/api/buy-requests/${item.id}`, { method: 'PATCH', body: JSON.stringify({ status: 'cancelled' }) }); setCancelling(null); notify('Запит скасовано'); load() }
    const editRequest = async (item: BuyRequest) => {
        try {
            const [detail, result] = await Promise.all([request(`/api/buy-requests/${item.id}`), request(`/api/buy-requests/${item.id}/offers`)])
            setEditLocked(result.offers.length > 0); setEditing(detail.buyRequest)
        } catch (error) { notify((error as Error).message) }
    }
    const copyRequest = async (item: BuyRequest) => {
        try { const detail = await request(`/api/buy-requests/${item.id}`); setCopying(true); setEditLocked(false); setEditing(detail.buyRequest) }
        catch (error) { notify((error as Error).message) }
    }
    ...
    return <section className="content"><div className="view-header"><div><span className="eyebrow">Мій попит</span><h1>Мої запити</h1><p className="view-subtitle">Переглядайте пропозиції продавців і приймайте їх повністю або частково.</p></div><button className="outline-button" onClick={load} disabled={loading}>↻ Оновити</button></div>
        {loading ? <div className="loading" role="status">Завантаження запитів…</div> : loadError ? <div className="empty-state" role="alert"><h3>Не вдалося завантажити запити</h3><p>{loadError}</p><button className="primary-button compact" onClick={load}>Спробувати ще раз</button></div> : <div className="request-list">{requests.map((item) => <article key={item.id} className="request-card"><header><div><span className="eyebrow">{item.category.name} · {item.geoArea}</span><h3>{item.title}</h3></div><span className="status status-active">{REQUEST_STATUS[item.status] ?? item.status}</span></header>
            ...
            <div className="request-actions"><button className="outline-button" onClick={() => setDetails(item)}>Деталі</button><button className="outline-button" onClick={() => toggle(item.id)}>Пропозиції{offers[item.id] ? ` (${offers[item.id].length})` : ''}</button>
                {(item.status === 'open' || item.status === 'partially_fulfilled') && <button className="outline-button" onClick={() => editRequest(item)}>Редагувати</button>}
                {(item.status === 'open' || item.status === 'partially_fulfilled') && <button className="outline-button" onClick={() => setCancelling(item)}>Скасувати</button>}</div>
            {(item.status === 'cancelled' || offers[item.id]?.length) && <button className="outline-button compact" onClick={() => copyRequest(item)}>Створити копію</button>}
            {expanded === item.id && <div className="offer-list">{(offers[item.id] ?? []).map((offer) => <div key={offer.id} className="offer-row">
                ...
                {(offer.status === 'submitted' || offer.status === 'partially_accepted') && <button className="primary-button compact" onClick={() => setAccepting({ offer, buyRequest: item })}>Прийняти {offer.quantity - offer.acceptedQuantity} {offer.unit}</button>}
                <button className="outline-button compact" onClick={() => openOfferChat(offer)}>♧ Чат</button>
        ...
        </article>)}{!requests.length && <Empty text="У вас ще немає запитів" action="Створити запит" onAction={create} />}</div>}{details && <RequestDetailsDialog item={details} onClose={() => setDetails(null)} />}{editing && <div className="modal-backdrop"><div className="detail-modal request-edit-modal"><BuyRequestForm categories={categories} buyRequest={editing} copy={copying} locked={editLocked} onCancel={() => { setEditing(null); setCopying(false) }} onSaved={() => { const copied = copying; setEditing(null); setCopying(false); notify(copied ? 'Новий запит створено з копії' : 'Запит оновлено'); load() }} /></div></div>}{cancelling && <ConfirmRequestCancellation item={cancelling} onClose={() => setCancelling(null)} onConfirm={() => cancel(cancelling).catch((error) => notify((error as Error).message))} />}{accepting && <AcceptOfferDialog offer={accepting.offer} buyRequest={accepting.buyRequest} onClose={() => setAccepting(null)} onConfirm={(quantity) => accept(accepting.offer, quantity)} />}</section>
}
