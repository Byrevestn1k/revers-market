...
import { findSettlements, hereRequest, joinHouseNumber, matchesHouseNumber, splitHouseNumber, matchesSettlement, matchesStreet, normalizePlace, resolveSettlement, savedAddressParts, searchStreets, settlementLabel, type HereItem, type SelectedSettlement, type Settlement } from './settlement-model'
...
export default function AddressInput({ initialValue = '', value, onChange, onBusyChange, name = 'exactAddress', requireStreet = true, openRequest = 0 }: Props) {
    ...
    const houseNumber = joinHouseNumber(house, letter)
    ...
    const begin = () => {
        ...
        const houseParts = splitHouseNumber(parts.house)
        setStreet(parts.street); setHouse(houseParts.number); setLetter(houseParts.letter)
    ...
    const choose = async (item: Settlement | null) => {
        const current = ++operation.current
        setSelected(item); setStreet(''); setHouse(''); setLetter(''); setStreetChosen(false); setStreetResults([]); setError(''); setUnconfirmed(false)
        if (!item) return
        setBusy(true)
    ...
    const saveAddress = (coordinates: Coordinates | null) => {
        ...
        const address = street.trim() ? [selected.name, street.trim(), houseNumber, selected.district, selected.region].filter(Boolean).join(', ') : ''
    ...
    const apply = async () => {
        ...
        const address = [selected.name, street.trim(), houseNumber, selected.district, selected.region].filter(Boolean).join(', ')
        ...
        if (street.trim()) {
            try {
                ...
                const hereAddress = [selected.name, street.trim(), houseNumber, selected.type === 'city' ? '' : selected.hereDistrict || selected.district, selected.region].filter(Boolean).join(', ')
                ...
                const matches = items.filter(item => item.position && matchesSettlement(item, selected) && matchesStreet(item.address?.street, street) && (house.trim() ? item.resultType === 'houseNumber' && matchesHouseNumber(item.address?.houseNumber, houseNumber) : item.resultType === 'street'))
                if (matches.length === 1) coordinates = { latitude: matches[0].position!.lat, longitude: matches[0].position!.lng }
                else if (!coordinates) throw new Error('Не вдалося точно визначити місце цієї адреси на мапі HERE. Це не означає, що будинку немає. Якщо адресу введено правильно, збережіть її без координат; точку можна вказати вручну на мапі.')
            } catch (error) {
    ...
    return <div className="address-editor"><input type="hidden" name={name} value={value?.address ?? saved} />
        ...
        {open && <div className="address-dialog-backdrop" onMouseDown={close}><div ref={popup} className="address-popup" role="dialog" aria-modal="true" aria-label="Вибір адреси" onMouseDown={event => event.stopPropagation()}>
            ...
            <div className="address-street-row"><label>Вулиця<input value={street} disabled={busy || !selected} maxLength={200} autoComplete="off" placeholder="Назва вулиці" onKeyDown={event => { if (event.key === 'Enter') event.preventDefault() }} onChange={event => { operation.current++; setStreet(event.target.value); setHouse(''); setLetter(''); setStreetChosen(false); setStreetResults([]); setError('') }} />
            ...
            </label><label>Будинок<input value={house} disabled={busy || !selected || !street.trim()} maxLength={40} placeholder="12" onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); void apply() } }} onChange={event => { const parts = splitHouseNumber(event.target.value); setHouse(parts.number); if (parts.letter) setLetter(parts.letter); setError(''); setUnconfirmed(false) }} /></label><label>Літера (необов’язково)<input value={letter} disabled={busy || !selected || !street.trim() || !house.trim()} maxLength={1} autoComplete="off" placeholder="А" onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); void apply() } }} onChange={event => { const next = event.target.value.trim(); if (!next || /^[a-zа-яіїєґ]$/iu.test(next)) { setLetter(next.toLocaleUpperCase('uk-UA')); setError(''); setUnconfirmed(false) } }} /></label></div>
