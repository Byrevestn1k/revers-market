...
const { default: SearchLocationControls } = require('../src/SearchLocationControls.tsx')
...
const publicCard = render(ProductCard, { product: { ...product, publicAddress: 'Рівне, Публічна адреса, 1' }, onOpen: noop })
assert.ok(publicCard.includes('Рівне, Публічна адреса, 1'))
...
const pointControls = render(SearchLocationControls, { location: { address: { address: '', city: '', coordinates: null }, busy: false }, nearbyDisabled: true, onPickPoint: noop })
assert.ok(pointControls.includes('Поставити точку на мапі'), 'Manual point selection must be available without an address')
const pickingControls = render(SearchLocationControls, { location: { address: { address: '', city: '', coordinates: null }, busy: false }, nearbyDisabled: true, onPickPoint: noop, pickingPoint: true })
assert.ok(pickingControls.includes('Скасувати вибір точки'))
assert.ok(pickingControls.includes('aria-pressed="true"'))
assert.doesNotMatch(noOrigin.match(/<button[^>]*map-nearby-button[^>]*>/)[0], /disabled/, 'Missing address must allow opening the address dialog instead of an inert nearby button')
assert.ok(controls.indexOf('Населений пункт') < controls.indexOf('Адреса пошуку'))
assert.ok(controls.indexOf('Адреса пошуку') < controls.indexOf('⌖ Моє місце'))
assert.ok(controls.includes('name="searchAddress"'), 'Search address is separate from the profile address field')
...
assert.ok(Object.hasOwn(mapProps, 'userId'), 'Map receives the user identity for private search-address initialization')
